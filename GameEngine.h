    ...
    while(getCurrentState()->getName() != "assign_reinforcement"){
            ...
            if(commandProcessor->validate(c) || c->getCommand().substr(0, 10) == "tournament"){
                ...
                if(arguments[0] == "loadmap"){
                    ...
                    // Write the results of the tournament to gamelog.txt
                    ofstream file("gamelog.txt", std::ios::app);
                    if (!file.is_open()) {
                        std::cerr << "ERROR: Could not open gamelog.txt" << endl;
                        return;
                    }
                    file << createTable(winners) << endl;
                    cout << createTable(winners) << endl;
                    file.close();
...
void GameEngine::processTournamentCommand(TournamentCommand* tc) {
    ...
    // Writing the tournament details to gamelog.txt
    file << "\n===== TOURNAMENT MODE ACTIVATED =====\n";
